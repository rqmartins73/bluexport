...
Before running `bluexport.sh`, first you must configure the file `bluexscrt` with your IBM Cloud Data (you can use bluexscrt_config.sh to help you create the bluexscrt file), and file `bluexport.conf` with the paths. File `bluexport.conf` must reside in your $HOME folder.
